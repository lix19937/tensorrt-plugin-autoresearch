...
Based on Karpathy's autoresearch — `constraint + mechanical metric + autonomous iteration = compounding gains`. So follow the roadmap: **Baseline → Modify → Verify/Metric → Keep/Discard → Repeat forever**.    
<br>
...
LOOP (N iterations or until done):
  ...
  5. Run mechanical verification (align, benchmarks)
  6. If improved → keep. If worse/compile error/runtime error → git revert/roll back. If crashed → fix or skip or exit.
  7. Log the result
...
## 4 [More to see](https://github.com/lix19937/llm-deploy/blob/master/auto_evolution.md)   
