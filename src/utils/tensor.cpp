...
Tensor::Tensor(Coords const& shape, DeviceType deviceType, nvinfer1::DataType dataType, std::string const& name) {
  ...
  if (deviceType == DeviceType::kCPU) {
    ...
    LOG_DEBUG(
      "Tensor %s of shape %s with size %ld bytes (%.2f MB) allocated on CPU",
      name.c_str(),
      shape.formatString().c_str(),
      memoryCapacity,
      utils::toMB(memoryCapacity));
  } else if (deviceType == DeviceType::kGPU) {
    CUDA_CHECK(cudaMalloc(&data, memoryCapacity));
    if (dataType == DataType::kHALF) {
      ::utils::fill_device_buffer<half>(data, shape.volume(), -1.f, 1.f, false);
    } else if (dataType == DataType::kFLOAT) {
      ::utils::fill_device_buffer<float>(data, shape.volume(), -1.f, 1.f, false);
    }

    LOG_DEBUG(
      "Tensor %s of shape %s with size %ld bytes (%.2f MB) allocated on GPU",
      name.c_str(),
      shape.formatString().c_str(),
      memoryCapacity,
      utils::toMB(memoryCapacity));
  ...
  {
    CUDA_CHECK(cudaHostAlloc(&h_data, memoryCapacity, cudaHostAllocMapped));
    CUDA_CHECK(cudaHostGetDevicePointer(&data, h_data, 0));
    LOG_DEBUG(
      "Tensor %s of shape %s with size %ld bytes (%.2f MB) allocated on SDRAM",
      name.c_str(),
      shape.formatString().c_str(),
      memoryCapacity,
      utils::toMB(memoryCapacity));
...
float Tensor::getAbsDiff(const Tensor& a, const Tensor& b) {
  float max = -FLT_MAX;

  auto v_a = a.getShape().volume();
  auto v_b = a.getShape().volume();
  if (v_a != v_b)
    return max;
  if (a.mDataType != b.mDataType)
    return max;

  if (a.mDataType == DataType::kHALF) {
    auto pa = (half*)(a.rawPointer());
    auto pb = (half*)(b.rawPointer());
    for (int i = 0; i < v_a; ++i) {
      float dt = std::abs(float(pa[i]) - float(pb[i]));
      if (dt > max) {
        max = dt;
      }
    }
  } else if (a.mDataType == DataType::kFLOAT) {
    auto pa = (float*)(a.rawPointer());
    auto pb = (float*)(b.rawPointer());
    for (int i = 0; i < v_a; ++i) {
      float dt = std::abs(pa[i] - pb[i]);
      if (dt > max) {
        max = dt;
      }
    }
...
void* Tensor::hostPointer() noexcept {
  return h_data;
}

void const* Tensor::hostPointer() const noexcept {
  return h_data;
}
...
void Tensor::releaseResource() {
  if (ownMemory) {
    if (mDeviceType == DeviceType::kCPU) {
      LOG_DEBUG(
        "Tensor %s of shape %s with size %ld bytes (%.2f MB) freed on CPU",
        mName.c_str(),
        mShape.formatString().c_str(),
        memoryCapacity,
        utils::toMB(memoryCapacity));
      CUDA_CHECK(cudaFreeHost(data));
    } else if (mDeviceType == DeviceType::kGPU) {
      LOG_DEBUG(
        "Tensor %s of shape %s with size %ld bytes (%.2f MB) freed on GPU",
        mName.c_str(),
        mShape.formatString().c_str(),
        memoryCapacity,
        utils::toMB(memoryCapacity));
      ...
      LOG_DEBUG(
        "Tensor %s of shape %s with size %ld bytes (%.2f MB) freed on SDRAM",
        mName.c_str(),
        mShape.formatString().c_str(),
        memoryCapacity,
        utils::toMB(memoryCapacity));
