...
class Coords {
 public:
  ...
  Coords(Coords const& coords) noexcept : mDims(coords.mDims), mNumDims(coords.mNumDims) {}
  ...
  Coords(nvinfer1::Dims const& dims) noexcept : mNumDims(dims.nbDims) {
  ...
  Coords(IT begin, IT end) : mNumDims(std::distance(begin, end)) {
  ...
  //! @throws std::runtime_error if number of dimensions exceeds kMAX_DIMS
  Coords(std::initializer_list<int64_t> init) : Coords(init.begin(), init.end()) {}

  //! @brief Construct from vector
  //! @param vec Vector of dimensions
  //! @throws std::runtime_error if number of dimensions exceeds kMAX_DIMS
  Coords(std::vector<int64_t> const& vec) : Coords(vec.begin(), vec.end()) {}

  //! @brief Get number of dimensions
  //! @return Number of dimensions
  int32_t getNumDims() const noexcept { return mNumDims; }
...
class Tensor {
 public:
   ...
   */
  Tensor(
    void* data, Coords const& extent, DeviceType deviceType, nvinfer1::DataType dataType, std::string const& name = "");
  ...
  //! Get CPU-SIDE pointer
  void const* hostPointer() const noexcept;
  void* hostPointer() noexcept;
  ...
  static float getAbsDiff(const Tensor& a, const Tensor& b);
