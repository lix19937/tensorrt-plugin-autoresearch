...
template <typename T>
void buildStringRecursive(
  std::stringstream& ss,
  T const* data,
  Coords const& shape,
  std::array<int64_t, kMAX_DIMS> const& strides,
  size_t& offset,
  int32_t dimension,
  size_t maxWidth,
...
std::string formatString(Tensor const& tensor) {
  ...
  if (tensor.getDeviceType() == DeviceType::kCPU) {
    ...
    CUDA_CHECK(
      cudaMemcpy(cpuTensor->rawPointer(), tensor.rawPointer(), cpuTensor->getMemoryCapacity(), cudaMemcpyDeviceToHost));
